#Small function required by frailtySim function
length_notInf<-function(vector){
...
#Setting up teh object class for the output
setClass("frailtySim", representation(indiv_hfh="vector", death="vector", t_death= "vector",  t_hfh= "matrix", 
treatment="vector", followup="vector", n="numeric", n_hfh="numeric", n_death="numeric", 
timeAtRisk="numeric"))

#This is the main function for simulating data with a joint frailty link between non-fatal and fatal events
simulateJF<-function(n.obs, p.active, hr.death, hr.hfh, rate.control.d, rate.control.hfh, 
length.fu,  frailty.scale, frailty.exponent) {


	#Checking sanity for censoring distribution and fraitly distribution
  ...
  if(frailty.scale<0){
	...
	frailty.hfh<-rgamma(n=n.obs,scale=frailty.scale, shape=frailty.shape) 
	...
	n_patients_hfh<-length(t.hfh.matrix[,1][t.hfh.matrix[,1]!=Inf])
  indiv_hfh<-apply(t.hfh.matrix,1, length_notInf)


	#Outputting information 
	output<-new("frailtySim",t_death=t.death.matrix,death=death, t_hfh=t.hfh.matrix, 
	treatment=treatment, n=length(treatment), followup=follow.up, n_hfh=n_hfh, 
	n_death=n_death, timeAtRisk=sum(pmin(length.fu, t.death.matrix)), indiv_hfh=indiv_hfh) 
	return(output)
}



#Transform JF takes the data output by fraitlySim, and converts it into a format that would be suitable 
#for analysis (e.g. by functions of Rondeau et al.). 

transform_jf<-function(jf_data) {
  
n.obs<-slot(jf_data,"n")
##Formatting data to be compliant with joint frailty programmes
t_hfh<-data.frame(cbind(matrix(1:n.obs, nrow=n.obs), slot(jf_data, "t_hfh")))
colnames(t_hfh)[2:3]<-c("next.hfh.col.0", "next_hfh.col.100")

#Reshaping HFH data into long format 
hfh_long<-reshape(t_hfh, idvar="subject",ids=t_hfh$V1, times=colnames(t_hfh)[2:dim(t_hfh)[2]], timevar="rep", 
                  direction="long", varying=list(colnames(t_hfh)[2:dim(t_hfh)[2]]))

hfh_long<-data.frame(cbind(hfh_long$subject, hfh_long$next.hfh.col))
...
death<-data.frame(cbind(1:n.obs, slot(jf_data, "t_death"), rep(0,times=n.obs)))
names(death)<-c("subject", "time", "death")
death$death[death$time!=Inf]<-1
death$time[death$time==Inf]<-slot(jf_data, "followup")[death$time==Inf]
death$hfh<-0

#Putting HFH and death data together, then merging in treatment codes 
treatment<-data.frame(cbind(1:n.obs, slot(jf_data, "treatment")))
